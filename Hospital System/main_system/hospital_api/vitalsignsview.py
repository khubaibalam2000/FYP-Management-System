...
def index(request):
    ...
    persons = VitalSigns.objects.all()
    serializer = VitalSignsSerializer(persons, many=True)
...
def get(request):
    ...
    try:
        person = VitalSigns.objects.get(id=request.GET['id'])
    except:
        return HttpResponse("Not Found", status=status.HTTP_404_NOT_FOUND)

    serializer = VitalSignsSerializer(person)
    return JsonResponse(serializer.data, safe=False)
